...
export default class GanonsTowerMap extends React.Component {
  constructor(props, context) {
    super(props, context);

    this.floors = ['B1', '1F', '2F', '3F', '4F', '5F', '6F', '7F'];
    this.mapName = 'ganons-tower';
  ...
  initMap(floorImages) {
    ...
    const map = this.map = new this.props.google.maps.Map(document.querySelector('#google-map'), {
      center: {lat: 55.7271, lng: -112.4561},
      zoom: 4,
    ...
    const allowedBounds = new this.google.maps.LatLngBounds( // eslint-disable-line
      new this.google.maps.LatLng(-40.9799, -179),
      new this.google.maps.LatLng(85, -45)
    );
